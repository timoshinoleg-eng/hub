...
function parseStartParam(sp) {
  ...
  return game ? { game, challenge: m[2] ? Number(m[2]) : null } : null;
}
function dailyGameForDate(date) {
  const pool = GAMES.filter((g) => g.enabled && g.cfg?.daily);
...
function renderMenu() {
  ...
  $('#streak-chip').innerHTML = summary.streak > 0
    ...
    : '<span class="streak-empty">Начни<br>серию</span>';
...
function tipSeen(id) {
  try { return localStorage.getItem('hub_tip_' + id) === '1'; } catch { return true; }
}
function markTip(id) {
...
function onMessage(e) {
  ...
  if (d.type === 'score') {
    const hadScore = state.score != null;
  ...
  if (d.type === 'finish') {
    ...
    const today = dailySeed();
    state.finishMeta = recordFinish(state.game, state.score ?? 0, today);
    track('finish', state.game.id, state.score);
    if (state.finishMeta.newBest) track('new_record', state.game.id, state.score);
    if (state.finishMeta.dailyAdvanced) track('daily_complete', state.game.id, state.finishMeta.streak);
    showResult();
  }
}
...
function showResult() {
  ...
  if (notifyBtn) {
    notifyBtn.onclick = async () => {
      if (!hasConsent()) {
        showConsent(() => doSubscribe(notifyBtn, g, score));
        return;
      }
      await doSubscribe(notifyBtn, g, score);
    };
  }
...
async function doSubscribe(btn, g, score) {
  ...
  btn.textContent = r.ok ? '✓ Подписка включена' : 'Не получилось';
  btn.disabled = r.ok;
...
function init() {
  ...
  track('open_bot');
  if (visit.firstVisit) track('first_visit');
  else if (visit.returning) track('return_visit', null, visit.daysAway);
