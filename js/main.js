...
function renderMenu(){
  const today=dailySeed();const summary=getSummary(today);const daily=dailyGameForDate(today);const all=visible(SHOW_ALL);const catalog=all.filter((g)=>g.id!==daily?.id);
  $('#streak-chip').innerHTML=`<strong>${summary.streak}</strong>🔥 серия`;
  if(daily){
    const pg=getGameProgress(daily.id);
    $('#daily-card').innerHTML=`<div class="daily-card" style="--d1:${daily.accent};--d2:${daily.accent2}"><div class="daily-top"><span class="daily-label"><img src="assets/brand/igroteka-mark.svg" alt=""> ВЫЗОВ ДНЯ</span><span class="daily-status">${summary.completedToday?'✓ серия сохранена':'только сегодня'}</span></div><div class="daily-main"><div class="daily-emoji">${art(daily,'daily-art')}</div><div class="daily-copy"><h2>${esc(daily.title)}</h2><p>${esc(daily.tagline)}</p><span>${esc(formatBest(daily,pg.best))}</span></div><button class="daily-play" id="daily-play" aria-label="Играть в ${esc(daily.title)}">›</button></div></div>`;
  ...
  for(const g of catalog){
    ...
    card.innerHTML=`<div class="gcard-top"><span class="ge">${art(g)}</span><span class="gbadge ${g.cfg?.daily?'daily':''}">${g.cfg?.daily?'✦ DAILY':esc(g.genre||g.length)}</span></div><span class="gt"><b>${esc(g.title)}</b><i>${esc(g.tagline)}</i></span><span class="gfoot"><span class="gbest">${esc(pg.best==null?g.length:formatBest(g,pg.best))}</span><span class="garrow">›</span></span>`;
    card.addEventListener('click',()=>openGame(g.id));list.appendChild(card);
  }
  $('#count').textContent=`${all.length} игр · ${catalog.length} ниже`;
...
function showResult(){
  ...
  box.innerHTML=`<div class="rcard" style="--result-glow:${g.accent||'#6d5cff'}">${meta.newBest?confettiHtml():''}${meta.newBest?'<div class="record-pill">✦ НОВЫЙ РЕКОРД</div>':''}<div class="result-emoji">${art(g,'result-art')}</div><div class="rttl">${esc(g.title)}</div><div class="result-score">${score}${g.unit?' <span style="font-size:14px;letter-spacing:0">'+esc(g.unit)+'</span>':''}</div><div class="rbest">${meta.newBest?'Лучший результат сохранён':esc(formatBest(g,meta.best))}</div>${duelLine}${dailyLine}<div class="rrow"><button class="btn primary" id="r-again">Ещё раз</button><button class="btn" id="r-share">Поделиться</button></div><div class="rrow"><button class="btn ghost" id="r-menu">К играм</button><button class="btn ghost" id="r-notify">Получать новинки</button></div><img class="rimg" alt="карточка результата"><div class="rhint" id="r-hint"></div></div>`;
